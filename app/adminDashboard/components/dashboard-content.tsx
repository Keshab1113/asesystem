...
import { Users, Activity, CheckCircle, XCircle, Eye, Edit, Trash2, Plus, Loader2, Copy } from "lucide-react"
import { ConfirmationDialog } from "./confirmation-dialog"
import { QuizDetailsModal } from "./quiz-details-modal"
import { QuizFormModal } from "./quiz-form-modal"
import { BulkActionsToolbar } from "./bulk-actions-toolbar"
import { useDebouncedValue } from "@/hooks/use-debounced-value"
import { AdvancedSearchFilters, type FilterState } from "./advanced-search-filters"
...
export function DashboardContent() {
  ...
  const [savedPresets, setSavedPresets] = useState<Array<{ name: string; filters: FilterState }>>([
    { name: "Active Programming", filters: { ...defaultFilters, status: "Active", subject: "Programming" } },
    { name: "Hard Difficulty", filters: { ...defaultFilters, difficulty: "hard" } },
  ])
  const [selectedQuizzes, setSelectedQuizzes] = useState<number[]>([])
  const [loadingStates, setLoadingStates] = useState<{ [key: number]: boolean }>({})
  const [deleteDialog, setDeleteDialog] = useState<{ open: boolean; quizId: number | null }>({
    open: false,
    quizId: null,
  })
  const [detailsModal, setDetailsModal] = useState<{ open: boolean; quiz: any }>({
    open: false,
    quiz: null,
  })
  const [formModal, setFormModal] = useState<{ open: boolean; quiz: any }>({
    open: false,
    quiz: null,
  })
  ...
  const filteredAndSortedQuizzes = useMemo(() => {
    const filtered = quizzes.filter((quiz) => {
      ...
      const matchesSubject = filters.subject === "all" || quiz.subject === filters.subject
      ...
      return matchesSearch && matchesStatus && matchesSubject && matchesDifficulty && matchesDateRange
    })
  ...
  const handleBulkAction = async (action: string, items: number[]) => {
    switch (action) {
      case "activate":
        setQuizzes((prev) => prev.map((quiz) => (items.includes(quiz.id) ? { ...quiz, status: "Active" } : quiz)))
        break
      case "deactivate":
        setQuizzes((prev) => prev.map((quiz) => (items.includes(quiz.id) ? { ...quiz, status: "Not Active" } : quiz)))
        break
      case "duplicate":
  ...
  const handleSavePreset = (name: string, filterState: FilterState) => {
  ...
  const handleLoadPreset = (filterState: FilterState) => {
  ...
  const handleToggleStatus = async (id: number) => {
    ...
    setQuizzes((prev) =>
      prev.map((quiz) =>
        quiz.id === id ? { ...quiz, status: quiz.status === "Active" ? "Not Active" : "Active" } : quiz,
      ),
    )
  ...
  const handleSaveQuiz = (quizData: any) => {
    if (quizData.id && quizzes.find((q) => q.id === quizData.id)) {
      ...
      setQuizzes((prev) => prev.map((quiz) => (quiz.id === quizData.id ? quizData : quiz)))
    } else {
      // Add new quiz
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">{activeQuizzes}</div>
          </CardContent>
        </Card>
      ...
      <div className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            <h2 className="text-2xl font-bold text-foreground text-balance">
              ...
              {filteredAndSortedQuizzes.length !== quizzes.length && (
                <span className="text-sm font-normal text-muted-foreground ml-2">of {quizzes.length} total</span>
              )}
            </h2>
            ...
            <Card key={quiz.id} className="hover:shadow-md transition-shadow">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <CardTitle className="text-lg text-card-foreground text-balance">{quiz.name}</CardTitle>
                  </div>
                  <Badge
                    variant={quiz.status === "Active" ? "default" : "secondary"}
                    className={
                      quiz.status === "Active" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
                    }
                  >
                    {quiz.status === "Active" ? (
              ...
              <CardContent className="space-y-4">
                ...
                <div className="text-xs text-muted-foreground">
                  Subject: {quiz.subject} • Difficulty: {quiz.difficulty} • Created: {quiz.createdDate}
                </div>
        ...
        {filteredAndSortedQuizzes.length === 0 && (
          <div className="text-center py-8 text-muted-foreground">
            No quizzes found matching your search criteria. Try adjusting your filters.
          </div>
        )}
      </div>
